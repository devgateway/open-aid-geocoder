...
/*Popup Data Entry*/
 class DataEntry extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = {showDeleteDialog:false, geocoding:Object.assign({},LOCATION_PROTOTYPE)};
  }


  locationClassChanged(e) {
    let locationClass = Constants.LOCATION_CLASS_LIST.find((item) => {if (item.code == e.target.value) return item;}) || null;
    let newGeocoding = Object.assign(this.state.geocoding, {locationClass: locationClass});
    this.setState(Object.assign(this.state, {geocoding: newGeocoding}));
    this.validateField(this.state.geocoding.locationClass, 'locationClass')  

  }

  exactnessChanged(e){
    let exactness = Constants.EXACTNESS_LIST.find((item) => {if (item.code == e.target.value) return item;}) || null;
    let newGeocoding = Object.assign(this.state.geocoding, {exactness: exactness});
    this.setState(Object.assign(this.state, {geocoding: newGeocoding}));
  ...
  activityDescriptionChanged(e){
    let newGeocoding = Object.assign(this.state.geocoding, {activityDescription: e.target.value});
    this.setState(Object.assign(this.state, {geocoding: newGeocoding}));
  ...
  validateField(value,elementId,validator){
     debugger;
      ...
      if (!validator){ //default validator
        validator=(val)=>{return val!=null}; //validator should return true if object is valid or false if object si not valid
      }
